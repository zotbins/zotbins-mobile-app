...
const ScanBottomSheetModal = forwardRef<
  BottomSheetModal,
  ScanBottomSheetModalProps
>(({ onClose, wasteObjects, image, onSuccessfulScan }, ref) => {
      ...
      if (image) {
        ...
        if (user) {
          ...
          const storageRef = storage().ref(
            `zotbins-waste-images/${user.uid}/${filename}`
          );
    ...
    >
      <BottomSheetView>
        <View className="items-center px-4 pb-4">
          <Text className="text-xl font-bold mb-3 text-darkGreen">
            {wasteObjects.length}{" "}
            {wasteObjects.length == 1 ? "Item Scanned" : "Items Scanned"}
          </Text>
          {/* render each result */}
          ...
          <View className="w-full flex-row justify-between mt-4 mb-2">
            ...
            >
              <Text className="text-darkGreen font-semibold text-md">
                Retake
              </Text>
            </Pressable>
            <Pressable
            ...
            >
              <Text className="text-darkGreen font-semibold text-md -translate-x-1">
                Submit
              </Text>
              <Ionicons
                name="arrow-forward"
